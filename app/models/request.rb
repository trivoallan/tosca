...
class Request < ActiveRecord::Base
  ...
  # self-explanatory
  TERMINEES = "requests.statut_id IN (#{Statut::CLOSED.join(',')})" unless defined? TERMINEES
  EN_COURS = "requests.statut_id IN (#{Statut::OPENED.join(',')})" unless defined? EN_COURS

  # See ApplicationController#scope
  def self.set_scope(contract_ids)
    scope = { :conditions =>
      [ 'requests.contract_id IN (?)', contract_ids ] }
  ...
  # /!\ Dirty Hack Warning /!\
  # We use finder for overused view mainly (requests/list)
  ...
  def state_at(t)
    ...
    return Request.new if t < self.created_on
  ...
  # <tt>Request.find(:all).each{|r| r.reset_elapsed }</tt>
  def reset_elapsed
    # clean previous existing elapsed
    Elapsed.destroy_all(['elapseds.request_id = ?', self.id])
  ...
  def commitment
    return nil unless contract_id && severite_id && typerequest_id
    self.contract.commitments.find(:first, :conditions =>
        {:typerequest_id => self.typerequest_id, :severite_id => self.severite_id})
  end
  ...
  def finish_first_comment
    self.first_comment.update_attribute :request_id, self.id
  end
